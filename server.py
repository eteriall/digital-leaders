...
tests = {
    1: {
        'Вопрос1': [[1, 0, 0, 0], ['D', '2', '3', '4']],
        'Вопрос2': [[0, 1, 0, 0], ['1', 'D', '3', '4']],
        'Вопрос3': [[0, 0, 1, 0], ['1', '2', 'D', '4']],
        'Вопрос4': [[0, 0, 0, 1], ['1', '2', '3', 'D']],
        'Вопрос5': [[0, 0, 0, 1], ['1', '2', '3', 'D']],
    },
    2: {
        'Вопрос1': [[1, 0, 0, 0], ['D', '2', '3', '4']],
        'Вопрос2': [[0, 1, 0, 0], ['1', 'D', '3', '4']],
...
def test(lesson):
    if "acc" in session and session['acc'] != "-1":
        if flask.request.method == 'POST':
            proc = int(sum(list(map(int, flask.request.form.values()))) * 100 / 5)
            if proc >= 80:
                con = sqlite3.connect(path.join(ROOT, "web_db.db"))
                cur = con.cursor()
                cur.execute(
